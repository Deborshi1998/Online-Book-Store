...
#Unique Token for user session generation. 
def generate_session_token(length=10):
    return ''.join(random.SystemRandom().choice([chr(i) for i in range(97, 123)] + [str(i) for i in range(10)]) for _ in range(length))

#Cross-site request forgery protection for signin.
@csrf_exempt
def signin(request):
...
#viewset class that handles the user registration login function.
